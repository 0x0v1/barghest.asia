...
title: "Data collection on Russian borders in device seizures aligned with SORM"
...
seoTitle: "PBAP call-log extraction in device seizures aligned with SORM"
seoDescription: "BARGHEST reconstructs a Russian border device seizure where contacts and call history were pulled over Bluetooth PBAP, reproduces it on a stock Pixel 8, and maps the identifier collection to SORM selectors."
image:
...
So the reproduction does not prove what happened to this specific handset; the overwrite and the missing packets put that beyond reach. What it proves is the cause and effect the case had to infer: a PBAP pull, and only a PBAP pull, leaves the pattern the seizure showed, while connection alone leaves nothing. The seizure supplies the effect on a real target, the reproduction supplies the cause on identical software, and the two only fit one story.
...
In that seizure the pull did not stand alone. It sat beside identifier collection, IMEI and SIM, and the two fit together: the contacts and call history map the target's social graph, most plausibly a screen for Ukrainian contacts and known actors, while the identifiers are the selectors a system like SORM targets. PBAP phonebook acquisition is a documented commercial capability, and it sits within a Russian forensic ecosystem, MKO Systems' Mobile Criminalist, which shares its origins with Oxygen Forensics and is used by Russian law enforcement, that this seizure fits. We prove the handset side and are explicit that the rest is context.
